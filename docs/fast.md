...
```sh
podman logs -f olcrtc-server-<суффикс>   # имя из строки Container name при запуске srv.sh
podman logs -f olcrtc-client            # имя из вывода cnc.sh
```

### Остановить

Имя контейнера сервера скрипт печатает при запуске (`Container name: olcrtc-server-…`). Подставь своё:

```sh
podman stop olcrtc-server-qb3jry3k
podman stop olcrtc-client
```

### Перезапустить (просто запусти скрипт снова)

Каждый запуск `./script/srv.sh` создаёт **новый** контейнер со **случайным** суффиксом в имени (`olcrtc-server-<8 символов>`). Предыдущие контейнеры **не** удаляются и **не** останавливаются автоматически — если их не гасить, несколько серверов могут работать параллельно.

Посмотреть все контейнеры olcrtc на машине:

```sh
podman ps -a --filter "name=olcrtc-"
```

Остановить и удалить лишние (подставь имена из колонки `NAMES`):

```sh
podman rm -f olcrtc-server-старый1 olcrtc-server-старый2
```

Флаг `-f` сначала останавливает, потом удаляет контейнер.
